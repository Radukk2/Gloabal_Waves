	...
	public ArrayList<String> SearchSongs(LibraryInput libraryInput, Comanda comanda) {
		...
		for (SongInput song : songs) {
			...
			if (comanda.getFilters().getName() != null && !song.getName().startsWith(comanda.getFilters().getName()))
				results.remove(song.getName());
			if (comanda.getFilters().getLyrics() != null && !song.getLyrics().toLowerCase().contains(comanda.getFilters().getLyrics().toLowerCase()))
				results.remove(song.getName());
...
//		searchCommands.checkIfPodcastWasLoaded(user, comm);
...
//					searchCommands.checkIfPodcastWasLoaded(user, comm);
	...
	public void checkIfPodcastWasLoaded(UserCommands userCommands, Comanda comanda) {
		if (userCommands.getLastCommand() == null)
			return;
		if (userCommands.getLastCommand().equals("load"))
			if (userCommands.getTrack().equals("podcast")) {
				userCommands.getPastPodcasts().add(userCommands.getPodcast());
				for (Podcast podcast : userCommands.getPastPodcasts())
					if (userCommands.getStats().isPaused() == false)
						if (userCommands.getStats().isPaused() == false) {
							if (userCommands.getStats().getRemainedTime() - comanda.getTimestamp() + userCommands.getLastTimestamp() > 0) {
								userCommands.getStats().setRemainedTime(userCommands.getStats().getRemainedTime() - comanda.getTimestamp() + userCommands.getLastTimestamp());
								userCommands.getPastPodcasts().get(userCommands.getPastPodcasts().size() - 1).setRemainingTime(userCommands.getStats().getRemainedTime());
							}
							else {
								EpisodeInput episodeInput = new EpisodeInput();
								boolean ok = false;
								if (userCommands.getPodcast().getPodcastInput().getEpisodes() == null)
									return;
								for (EpisodeInput episodeInput1 : userCommands.getPodcast().getPodcastInput().getEpisodes()) {
									int index = userCommands.getPodcast().getPodcastInput().getEpisodes().indexOf(episodeInput1);
									if (episodeInput1.getName().equals(userCommands.getCurrentEpisode())) {
										for (int i = index + 1; i < userCommands.getPodcast().getPodcastInput().getEpisodes().size(); i++) {
											EpisodeInput episodeInput2 = userCommands.getPodcast().getPodcastInput().getEpisodes().get(i);
											userCommands.getStats().setName(episodeInput2.getName());
											userCommands.getStats().setRemainedTime(userCommands.getStats().getRemainedTime() + episodeInput2.getDuration());
											if (userCommands.getStats().getRemainedTime() - comanda.getTimestamp() + userCommands.getLastTimestamp() > 0) {
												userCommands.getStats().setRemainedTime(userCommands.getStats().getRemainedTime() - comanda.getTimestamp() + userCommands.getLastTimestamp());
												ok = true;
												break;
								...
								if (ok == false) {
									userCommands.getStats().setRemainedTime(0);
									userCommands.getStats().setName("");
								}
								int index = userCommands.getPastPodcasts().size();
							}
						}
			}
	}
